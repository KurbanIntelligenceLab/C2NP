...
from models.task_2.diffcsp_model import DiffCSP_Task2
...
SEEDS = config.get_seeds_for_model("diffcsp")
BATCH_SIZE = config.get_batch_size_for_model("diffcsp")
GRAD_CLIP = config.get_grad_clip_for_model("diffcsp")
...
for SEED in SEEDS:
    ...
    # Output directory
    out_dir = config.get_output_dir("diffcsp", SEED)
    ...
    def evaluate(loader):
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="Evaluating"):
                ...
                # Generate samples
                try:
                    l_pred, sg_logits = model.sample(batch)
