...
from models.task_2.cdvae_model import CDVAE_Task2
...
def run_epoch(model, loader, optimizer=None, train=False, device="cpu"):
    if train:
        ...
        model.eval()
    tot = reg_tot = cls_tot = kl_tot = correct = n_graph = 0
    ...
    for batch in pbar:
        ...
        with torch.set_grad_enabled(train):
            l_pred, sg_logits, mu, logvar = model(batch)
            loss_r = reg_loss(l_pred, l_true)
            loss_c = cls_loss(sg_logits, sg_true)
            loss_k = kld(mu, logvar)
            loss = loss_r + CLS_W * loss_c + KL_W * loss_k
            if train:
                optimizer.zero_grad()
                loss.backward()
...
for SEED in [50, 60]:
    ...
    # Output directory
    out_dir = os.path.join("results", "task_2", "cdvae", str(SEED))
    ...
    model = CDVAE_Task2(
        hidden_dim=4, latent_dim=4, num_layers=1, cutoff=5.0, num_spacegroups=NUM_SG
    ).to(DEVICE)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        train_metrics = run_epoch(model, train_loader, optimizer, True, DEVICE)
        val_metrics = run_epoch(model, val_loader, None, False, DEVICE)
        ...
        history.append(
            {
                ...
                "train_reg": train_metrics[1],
                "train_cls": train_metrics[2],
                "train_kl": train_metrics[3],
                "train_acc": train_metrics[4],
                "val_loss": val_metrics[0],
                "val_reg": val_metrics[1],
                "val_cls": val_metrics[2],
                "val_kl": val_metrics[3],
    ...
    def evaluate(loader):
        se_accum = correct_sg = correct_joint = n = 0
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                # Forward pass
                try:
                    l_pred, sg_logits, *_ = model(batch)
                    ...
                    all_joint_correct[batch_idx : batch_idx + B] = (
                        sg_pred == sg_true
                    ) & (se <= 0.25).all(dim=1)
                except Exception as e:
                    print(f"Error in model forward pass for batch: {e}")
    ...
    id_metrics = evaluate(id_loader)
    ood_metrics = evaluate(ood_loader)
    # Save test results
    df = pd.DataFrame(
        [
            {
