"""
Central configuration for Task 1 training.

All training hyperparameters, model architectures, and data settings are defined here.
All training scripts in task_1/ should import and use these configurations.
"""
...
DATALOADER_NUM_WORKERS = 4  # For dataset loading
TRAIN_LOADER_NUM_WORKERS = 0  # For training data loaders
EVAL_LOADER_NUM_WORKERS = 0  # For evaluation data loaders

# ═══════════════════════════════════════════════════════════════════════════════
# TRAINING HYPERPARAMETERS
# ═══════════════════════════════════════════════════════════════════════════════
BATCH_SIZE = 1
EVAL_BATCH_SIZE = 4  # Used for some models during evaluation
LEARNING_RATE = 1e-4
NUM_EPOCHS = 5

# Random seeds for different runs (can be model-specific)
DEFAULT_SEEDS = [42, 50, 60]
...
# OUTPUT PATHS
# ═══════════════════════════════════════════════════════════════════════════════
RESULTS_BASE_DIR = "results/task_1"
...
# Model-specific eval batch sizes
MODEL_EVAL_BATCH_SIZES = {
    "adit": 1,
    "cdvae": 1,
    "diffcsp": 1,
    "flowllm": 4,  # FlowLLM uses larger batch for eval
    "flowmm": 1,
    "mattergen": 1,
}
...
# ═══════════════════════════════════════════════════════════════════════════════
@dataclass
class Task1TrainingConfig:
    """Configuration for Task 1 training."""
    ...
    train_loader_num_workers: int = TRAIN_LOADER_NUM_WORKERS
    eval_loader_num_workers: int = EVAL_LOADER_NUM_WORKERS
    ...
    def get_eval_batch_size_for_model(self, model_name: str) -> int:
        """Get eval batch size for a specific model."""
        return MODEL_EVAL_BATCH_SIZES.get(model_name.lower(), self.eval_batch_size)
    ...
    @classmethod
    def default(cls) -> "Task1TrainingConfig":
