...
from models.task_1.mattergen_model import MatterGenUnitCell
...
def run_epoch(model, loader, optimizer=None, train=False, device=None):
    ...
    for data in pbar:
        ...
        noisy_pos = (
            torch.sqrt(alpha[model._batch_from_ptr(data.ptr)]) * data.pos
            + torch.sqrt(1 - alpha[model._batch_from_ptr(data.ptr)]) * noise
        )
...
for SEED in [42, 50, 60]:
    ...
    # Model, optimizer
    model = MatterGenUnitCell(
        atom_emb_dim=4, hidden_dim=4, num_layers=1, cutoff_radius=5.0, time_emb_dim=4
    ).to(DEVICE)
